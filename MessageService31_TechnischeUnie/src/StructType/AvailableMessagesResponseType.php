...
class AvailableMessagesResponseType extends AbstractStructBase
{
     ...
     * @param \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType[] $messageList
     */
    ...
    public static function validateMessageListForArrayConstraintsFromSetMessageList(?array $values = []): string
    {
        ...
        foreach ($values as $availableMessagesResponseTypeMessageListItem) {
            ...
            if (!$availableMessagesResponseTypeMessageListItem instanceof \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MessageList property can only contain items of type \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType[] $messageList
     * @return \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\AvailableMessagesResponseType
     */
     ...
     * @throws InvalidArgumentException
     * @param \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType $item
     * @return \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\AvailableMessagesResponseType
     */
    public function addToMessageList(\Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType) {
            throw new InvalidArgumentException(sprintf('The MessageList property can only contain items of type \Wefabric\MessageSender\MessageService31_TechnischeUnie\StructType\MessagePropertiesType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
